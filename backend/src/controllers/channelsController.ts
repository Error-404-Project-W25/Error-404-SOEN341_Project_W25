...
export const createChannel = async (
  ...
  const { team_id, creator_id, channelName, channelDescription, members } =
    req.body; // get channel info
...
export const addUserToChannel = async (
  ...
  try {
    ...
    team.channels
      .find((teamChannel) => teamChannel.id === channel_id)
      ?.members.push(userInTeam);
    await team.save();
    res.status(201).json({
